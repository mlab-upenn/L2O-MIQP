...
class LSTMNet(torch.nn.Module):
    ...
    def __init__(self,shape, layer_dim, activation=None):
        super(LSTMNet, self).__init__()
        self.shape = shape
        self.layer_dim = layer_dim
        self.activation = activation

        fc1 = torch.nn.Linear(shape[0], shape[1])
        lstm = torch.nn.LSTM(shape[1], shape[2], layer_dim, batch_first=True)
        fc2 = torch.nn.Linear(shape[2], shape[3])
        self.layers = [fc1, lstm, fc2]
        self.layers = torch.nn.ModuleList(self.layers)
    ...
    def forward(self, x):
        ...
        h0 = torch.zeros(self.layer_dim, x.size(0), self.shape[2]).requires_grad_().to(x.device)
        # Initialize cell state
        c0 = torch.zeros(self.layer_dim, x.size(0), self.shape[2]).requires_grad_().to(x.device)
        
        x, (hn, cn) = self.layers[1](x, (h0.detach(), c0.detach()))
        x = self.layers[-1](x) 
